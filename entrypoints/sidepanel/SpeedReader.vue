...
const focusColor = computed(() =>
  props.prefs.useThemeColor ? 'var(--color-content)' : props.prefs.color,
);
...
<template>
  <div class="h-full flex flex-col bg-app text-content">
    ...
    <header class="flex items-center gap-2 px-3 py-2 border-b border-line">
      ...
      <span class="text-xs text-muted tabular-nums">{{ Math.min(index + 1, total) }}/{{ total }}</span>
    </header>

    <!-- Stage -->
    <div class="flex-1 flex flex-col items-center justify-center px-4 select-none">
      <!-- context before -->
      <div v-if="prefs.contextWords > 0" class="text-muted text-center text-sm mb-3 leading-relaxed">
        {{ contextBefore.join(' ') }}
      </div>
      ...
      >
        <template v-if="current">
          <span>{{ current.text.slice(0, current.pivot) }}</span><span class="text-accent">{{ current.text.charAt(current.pivot) }}</span><span>{{ current.text.slice(current.pivot + 1) }}</span>
        </template>
        <span v-else class="text-muted text-base font-normal">No text to read.</span>
      </div>

      <!-- context after -->
      <div v-if="prefs.contextWords > 0" class="text-muted text-center text-sm mt-3 leading-relaxed">
        {{ contextAfter.join(' ') }}
      </div>
    </div>
    ...
    <div class="flex items-center justify-center gap-2 px-4 py-3">
      <button class="rounded-md border border-line w-9 h-9 hover:bg-surface" title="Restart" @click="stop">⏮</button>
      <button class="rounded-md border border-line w-9 h-9 hover:bg-surface" title="Previous word" @click="step(-1)">◀</button>
      <button
        class="rounded-md bg-accent hover:bg-accent-hover text-on-accent w-14 h-9 font-medium"
        :disabled="!total"
        @click="toggle"
      >{{ playing ? 'Pause' : 'Play' }}</button>
      <button class="rounded-md border border-line w-9 h-9 hover:bg-surface" title="Next word" @click="step(1)">▶</button>
    </div>

    <!-- Live prefs -->
    <div class="px-4 pb-4 space-y-2 border-t border-line pt-3 text-xs">
      <label class="flex items-center gap-2">
        <span class="w-16 text-muted">Speed</span>
        <input
      ...
      <label class="flex items-center gap-2">
        <span class="w-16 text-muted">Size</span>
        <input
      ...
      <label class="flex items-center gap-2">
        <span class="w-16 text-muted">Context</span>
        <input
      ...
      <div class="flex items-center gap-2">
        <span class="w-16 text-muted">Font</span>
        <select
          :value="prefs.fontFamily"
          class="flex-1 rounded-md border border-line bg-transparent px-2 py-1"
        ...
        <input
          ...
          class="h-7 w-9 rounded border border-line bg-transparent disabled:opacity-40"
